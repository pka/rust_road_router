...
impl<G: DijkstrableGraph, H: DijkstrableGraph, C: Deref<Target = G>> Server<G, H, C> {
    pub fn distance(&mut self, from: NodeId, to: NodeId) -> Option<Weight> {
        ...
        // let mut forward_routine = self.forward_dijkstra.query_generator(Query { from, to });
        // let mut backward_routine = self.backward_dijkstra.query_generator(Query { from: to, to: from });
